from generate_movie_db.constants import PLAYER_TO_MOVIE_TABLE, DB_NAME, ACTOR_ID, ACTOR_MOVIES, ACTOR_NAME
...
def get_actor_uid(actor_name: str) -> str:
    ...
    con = connect(DB_NAME)
    cur = con.cursor()
    cur.execute(f"SELECT {ACTOR_ID}, {ACTOR_NAME}, {ACTOR_MOVIES} FROM {PLAYER_TO_MOVIE_TABLE} WHERE {ACTOR_NAME} = {actor_name}")
    row = cur.fetchone()
    con.close()
    if not row:
...
def get_actor_movies(actor_uid: str) -> List[str]:
    ...
    con = connect(DB_NAME)
    cur = con.cursor()
    cur.execute(f"SELECT {ACTOR_ID}, {ACTOR_NAME}, {ACTOR_MOVIES} FROM {PLAYER_TO_MOVIE_TABLE} WHERE {ACTOR_ID} = {actor_uid}")
    row = cur.fetchone()
    con.close()
    if not row:
...
def get_actors_number() -> int:
    ...
    con = connect(DB_NAME)
    cur = con.cursor()
    cur.execute(f"SELECT COUNT(*) FROM {PLAYER_TO_MOVIE_TABLE}")
    row = cur.fetchone()
    con.close()
    if not row:
...
def instalize_min_heap() -> List[Tuple[Union[int, float], str]]:
    ...
    con = connect(DB_NAME)
    cur = con.cursor()
    cur.execute(f"SELECT * FROM {PLAYER_TO_MOVIE_TABLE}")
    while True:
